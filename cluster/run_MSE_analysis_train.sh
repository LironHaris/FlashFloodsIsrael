#!/bin/bash
#SBATCH --job-name=MSE_analysis_train
...
# 6. ניתוח MSE על סט ה-train עם מודל מאומן (best_model.pt) + ביקורת שימוש בסט
#    ללא אימון - דורש שהאימון עבור הקונפיג הזה כבר הושלם
CONFIG_PATH="${1:-configs/config.yml}"
python src/MSE_analysis_train.py --config "$CONFIG_PATH"
